...
The link for my github: https://github.com/ducminhngo871/Interpretable-MLAndShinyApps
...
In here, from the histogram plot, we can see that the shape is right-skewed with the majority of the residuals is from 0 to 0.2. 

From the boxplot, we also see that the mean residual is around 0.05.
...
In here, we can see that interest_rate is the most important variable, followed by sub_grade and open_il_24m. 
...
In here, I will choose 3 observations: observation 10, 100 and observation 200. 

```{r}
# Choose an observation
new_obs_1 <- lending_training %>% slice(10) 
new_obs_2 <- lending_training %>% slice(100) 
new_obs_3 <- lending_training %>% slice(200) 
...
pp_lasso1 <- predict_parts(explainer = rf_explain,
                          new_observation = new_obs_1,
                          type = "break_down") #default

pp_lasso2 <- predict_parts(explainer = rf_explain,
                          new_observation = new_obs_2,
                          type = "break_down") #default

pp_lasso3 <- predict_parts(explainer = rf_explain,
...
In here, we can see that observation 10 predicts bad, whereas observation 100 and 200 predicts good. 

For observation 10, we can see that total_bal_il attributes to the prediction with -0.126. 

For observation 100, interest_rate attributes the most with 0.057. In here, we can see that some of the attributes result in the negative number of the prediction such as all_util, total_bal_il or open_il_24m.  

For observation 200, interest_rate attributes the most with 0.028. 
...
I believe that knowing the importance variables to see which one is more important will be extremely important for future machine learning projects. As if I have a dataset that has too many variables, using the importance variables will be really helpful to choose the best variables to run further model. 
...
```{r}
saveRDS(rf_final, "rf_final_fit.rds")
```
